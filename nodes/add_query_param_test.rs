// Separate test file: nodes/add_query_param_test.rs. The generated service wires
// it into the crate via `#[cfg(test)] #[path="nodes/add_query_param_test.rs"] mod
// add_query_param_test;`. It reaches the node + SDK through `crate::` paths (this is
...
mod tests {
    ...
    use crate::add_query_param::add_query_param;
    ...
    #[test]
    fn test_appends_even_if_key_exists() {
        let ax = test_context();
        let out = add_query_param(&ax, qi("https://example.com/?a=1", "a", "2")).unwrap();
        assert_eq!(out.error, "");
        assert_eq!(out.url, "https://example.com/?a=1&a=2");
    }

    // Independent oracle: application/x-www-form-urlencoded serialization
    // (WHATWG URL Standard §5) encodes a literal space as '+'.
    #[test]
    fn test_space_encoded_as_plus() {
        let ax = test_context();
        let out = add_query_param(&ax, qi("https://example.com/", "x", "y z")).unwrap();
        assert_eq!(out.error, "");
        assert_eq!(out.url, "https://example.com/?x=y+z");
    }

    #[test]
    fn test_first_param_on_bare_url() {
        let ax = test_context();
        let out = add_query_param(&ax, qi("https://example.com/path", "q", "1")).unwrap();
        assert_eq!(out.error, "");
    ...
    fn test_empty_key_is_structured_error() {
        let ax = test_context();
        let out = add_query_param(&ax, qi("https://example.com/", "", "v")).unwrap();
    ...
    fn test_malformed_url_is_structured_error() {
        let ax = test_context();
        let out = add_query_param(&ax, qi("not a url", "a", "1")).unwrap();
