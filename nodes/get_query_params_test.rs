// Separate test file: nodes/get_query_params_test.rs. The generated service wires
// it into the crate via `#[cfg(test)] #[path="nodes/get_query_params_test.rs"] mod
// get_query_params_test;`. It reaches the node + SDK through `crate::` paths (this is
...
mod tests {
    ...
    use crate::get_query_params::get_query_params;
    ...
    #[test]
    fn test_order_and_duplicate_keys_preserved() {
        let ax = test_context();
        let out = get_query_params(&ax, ui("https://example.com/?a=1&b=2&a=3")).unwrap();
        assert_eq!(out.error, "");
        assert_eq!(out.pairs.len(), 3);
        assert_eq!((out.pairs[0].key.as_str(), out.pairs[0].value.as_str()), ("a", "1"));
        assert_eq!((out.pairs[1].key.as_str(), out.pairs[1].value.as_str()), ("b", "2"));
        assert_eq!((out.pairs[2].key.as_str(), out.pairs[2].value.as_str()), ("a", "3"));
    }

    // Independent oracle: application/x-www-form-urlencoded decoding rules
    // (WHATWG URL Standard §5) — literal '+' decodes to space, but a
    // percent-escaped plus (%2B) decodes to a literal '+' character. These
    // two must NOT collapse to the same decoded value.
    #[test]
    fn test_plus_vs_percent_encoded_plus() {
        let ax = test_context();
        let out = get_query_params(&ax, ui("https://example.com/?x=a+b&y=a%2Bb")).unwrap();
        assert_eq!(out.error, "");
        assert_eq!(out.pairs.len(), 2);
        assert_eq!(out.pairs[0].value, "a b");
        assert_eq!(out.pairs[1].value, "a+b");
    }

    #[test]
    fn test_no_query_yields_empty_pairs() {
        let ax = test_context();
        let out = get_query_params(&ax, ui("https://example.com/path")).unwrap();
        assert_eq!(out.error, "");
    ...
    fn test_malformed_url_is_structured_error() {
        let ax = test_context();
        let out = get_query_params(&ax, ui("not a url")).unwrap();
